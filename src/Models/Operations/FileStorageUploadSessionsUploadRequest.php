<?php

/**
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

declare(strict_types=1);

namespace Apideck\Unify\Models\Operations;

use Apideck\Unify\Utils\SpeakeasyMetadata;
class FileStorageUploadSessionsUploadRequest
{
    /**
     * ID of the record you are acting upon.
     *
     * @var string $id
     */
    #[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=id')]
    public string $id;

    /**
     * Part number of the file part being uploaded.
     *
     * @var float $partNumber
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=part_number')]
    public float $partNumber;

    /**
     *
     * @var string $requestBody
     */
    #[SpeakeasyMetadata('request:mediaType=*/*')]
    public string $requestBody;

    /**
     * ID of the consumer which you want to get or push data from
     *
     * @var ?string $consumerId
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-consumer-id')]
    public ?string $consumerId = null;

    /**
     * The ID of your Unify application
     *
     * @var ?string $appId
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-app-id')]
    public ?string $appId = null;

    /**
     * Provide the service id you want to call (e.g., pipedrive). Only needed when a consumer has activated multiple integrations for a Unified API.
     *
     * @var ?string $serviceId
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-service-id')]
    public ?string $serviceId = null;

    /**
     * The RFC3230 message digest of the uploaded part. Only required for the Box connector. More information on the Box API docs [here](https://developer.box.com/reference/put-files-upload-sessions-id/#param-digest)
     *
     * @var ?string $digest
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=digest')]
    public ?string $digest = null;

    /**
     * Include raw response. Mostly used for debugging purposes
     *
     * @var ?bool $raw
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=raw')]
    public ?bool $raw = null;

    /**
     * @param  string  $id
     * @param  float  $partNumber
     * @param  string  $requestBody
     * @param  ?string  $consumerId
     * @param  ?string  $appId
     * @param  ?string  $serviceId
     * @param  ?string  $digest
     * @param  ?bool  $raw
     * @phpstan-pure
     */
    public function __construct(string $id, float $partNumber, string $requestBody, ?string $consumerId = null, ?string $appId = null, ?string $serviceId = null, ?string $digest = null, ?bool $raw = false)
    {
        $this->id = $id;
        $this->partNumber = $partNumber;
        $this->requestBody = $requestBody;
        $this->consumerId = $consumerId;
        $this->appId = $appId;
        $this->serviceId = $serviceId;
        $this->digest = $digest;
        $this->raw = $raw;
    }
}