<?php

/**
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

declare(strict_types=1);

namespace Apideck\Unify\Models\Operations;

use Apideck\Unify\Utils\SpeakeasyMetadata;
class ProxyOptionsProxyRequest
{
    /**
     * Provide the service id you want to call (e.g., pipedrive). Only needed when a consumer has activated multiple integrations for a Unified API.
     *
     * @var string $serviceId
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-service-id')]
    public string $serviceId;

    /**
     * Downstream URL
     *
     * @var string $downstreamUrl
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-downstream-url')]
    public string $downstreamUrl;

    /**
     * ID of the consumer which you want to get or push data from
     *
     * @var ?string $consumerId
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-consumer-id')]
    public ?string $consumerId = null;

    /**
     * The ID of your Unify application
     *
     * @var ?string $appId
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-app-id')]
    public ?string $appId = null;

    /**
     * Specify which unified API to use for the connection lookup. Required for multi-API connectors (e.g., Workday) to ensure the correct credentials are used.
     *
     * @var ?string $unifiedApi
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-unified-api')]
    public ?string $unifiedApi = null;

    /**
     * Downstream authorization header. This will skip the Vault token injection.
     *
     * @var ?string $downstreamAuthorization
     */
    #[SpeakeasyMetadata('header:style=simple,explode=false,name=x-apideck-downstream-authorization')]
    public ?string $downstreamAuthorization = null;

    /**
     * @param  string  $serviceId
     * @param  string  $downstreamUrl
     * @param  ?string  $consumerId
     * @param  ?string  $appId
     * @param  ?string  $unifiedApi
     * @param  ?string  $downstreamAuthorization
     * @phpstan-pure
     */
    public function __construct(string $serviceId, string $downstreamUrl, ?string $consumerId = null, ?string $appId = null, ?string $unifiedApi = null, ?string $downstreamAuthorization = null)
    {
        $this->serviceId = $serviceId;
        $this->downstreamUrl = $downstreamUrl;
        $this->consumerId = $consumerId;
        $this->appId = $appId;
        $this->unifiedApi = $unifiedApi;
        $this->downstreamAuthorization = $downstreamAuthorization;
    }
}